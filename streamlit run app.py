...
st.title("📊 كاشف فرص العملات الرقمية + RSI (CryptoCompare API)")
...
API_KEY = "c9d4fdc4a8bbdc7ce6d2aee814d87e46153d13a0f287a72394cee48cb8db0e1c"
...
def fetch_ohlc_daily(symbol):
    try:
        url = f"https://min-api.cryptocompare.com/data/v2/histoday?fsym={symbol.upper()}&tsym=USD&limit=200"
        r = requests.get(url, headers=HEADERS, timeout=10).json()
        data = r.get("Data", {}).get("Data", [])
        if not data:
            return pd.DataFrame()
        df = pd.DataFrame(data)
        df = df.rename(columns={"time":"time", "open":"open", "high":"high", "low":"low", "close":"close", "volumefrom":"volume"})
